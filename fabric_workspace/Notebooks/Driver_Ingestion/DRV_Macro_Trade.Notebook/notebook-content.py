...
file_path = (
    ...
    "Macroeconomic_Trade_Investment_Predictive_GlobalData.xlsx"
)
...
df_raw.columns = ["Country", "Indicator", "Currency", "Unit"] + list(df_raw.columns[4:])
...
df_raw["Country"] = df_raw["Country"].replace("", np.nan).replace("NaN", np.nan).ffill()
...
df_unpivot = df_raw.melt(
    id_vars=["Country", "Indicator", "Currency", "Unit"],
...
df_unpivot = df_unpivot[
    ["Country", "Indicator", "Currency", "Unit", "Date", "Value"]
]
...
# (assumes Macro_Trade_GD_World exists)
# -----------------------------------------------------------------------------
...
filled_df = filled_df.withColumn(
    "Currency",
    last("Currency", ignorenulls=True).over(
        Window.partitionBy("Country", "Indicator")
              .orderBy("Date")
              .rowsBetween(Window.unboundedPreceding, 0)
    )
...
filled_df.write.format("delta").mode("overwrite").saveAsTable("Sales_Forecasting.bronze.DRV_Macro_Trade")
