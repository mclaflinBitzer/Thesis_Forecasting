...
parquet_dir = forecast_parquet_base_dir #stored in Config file
...
if driver_status == 'Manual_Drivers':
    ...
    selected_driver_dir = config_obj['xgboost_automated_features_output_dir']

parquet_dir = config_obj['forecast_parquet_base_dir']
parquet_dir = parquet_dir + driver_status + "/"

TFT_forecast_dir = parquet_dir + "TFT_Forecast_Output.parquet"
...
# FORECAST_HORIZON | SEASONAL_PERIODS | MIN_TRAIN | N_OPTUNA_TRIALS | OPTUNA_SEED | ENCODER_LENGTH | driver_table
...
if run_tft:
    if rerun_historical_forecasts:
        print(f"overwriting parquet with new historical forecast values: {TFT_forecast_dir}")
        forecasts.write.mode('overwrite').parquet(TFT_forecast_dir)
        ...
        print(f"appending new forecast values to the existing parquet {TFT_forecast_dir}")
        ## making sure that if there is an existing forecast for the most recent run that it is overwritten

        # FORECAST VALUES
        existing = spark.read.parquet(TFT_forecast_dir).cache()
        existing.count()

        updated = (
            existing.join(
                forecasts.select(*ACT_GRP_COLS, "Training_End_Date"),
                [*ACT_GRP_COLS, "Training_End_Date"],
                "left_anti"
            )
            .unionByName(forecasts)
        )

        updated.write.mode("overwrite").parquet(TFT_forecast_dir)


        # STATIC VARIABLE EVALUATION
        existing = spark.read.parquet(TFT_static_dir).cache()
        existing.count()

        updated = (
            existing.join(
                static_importance.select(*ACT_GRP_COLS, "Training_End_Date"),
                [*ACT_GRP_COLS, "Training_End_Date"],
                "left_anti"
            )
            .unionByName(static_importance)
        )

        updated.write.mode("overwrite").parquet(TFT_static_dir)


        # DECODER EVALUATION
        existing = spark.read.parquet(TFT_decoder_dir).cache()
        existing.count()

        updated = (
            existing.join(
                decoder_importance.select(*ACT_GRP_COLS, "Training_End_Date"),
                [*ACT_GRP_COLS, "Training_End_Date"],
                "left_anti"
            )
            .unionByName(decoder_importance)
        )

        updated.write.mode("overwrite").parquet(TFT_decoder_dir)


        # ATTENTION EVALUATION
        existing = spark.read.parquet(TFT_attention_dir).cache()
        existing.count()

        updated = (
            existing.join(
                attention.select(*ACT_GRP_COLS, "Training_End_Date"),
                [*ACT_GRP_COLS, "Training_End_Date"],
                "left_anti"
            )
            .unionByName(attention)
        )

        updated.write.mode("overwrite").parquet(TFT_attention_dir)
