...
if prophet_run:
    ...
    if rerun_historical_forecasts:
        ...
        existing = spark.read.parquet(Prophet_dir).cache()
        existing.count()

        updated = (
            existing.join(
                prophet_output.select(*ACT_GRP_COLS, "Training_End_Date"),
                [*ACT_GRP_COLS, "Training_End_Date"],
                "left_anti"
            )
            .unionByName(prophet_output)
        )

        updated.write.mode("overwrite").parquet(Prophet_dir)
...
if sarimax_run:
    ...
    if rerun_historical_forecasts:
        ...
        ## making sure that if there is an existing forecast for the most recent run that it is overwritten
        existing = spark.read.parquet(Sarimax_dir).cache()
        existing.count()

        updated = (
            existing.join(
                sarimax_output.select(*ACT_GRP_COLS, "Training_End_Date"),
                [*ACT_GRP_COLS, "Training_End_Date"],
                "left_anti"
            )
            .unionByName(sarimax_output)
        )

        updated.write.mode("overwrite").parquet(Sarimax_dir)
...
if arimax_run:
    ...
    if rerun_historical_forecasts:
        ...
        ## making sure that if there is an existing forecast for the most recent run that it is overwritten
        existing = spark.read.parquet(Arimax_dir).cache()
        existing.count()

        updated = (
            existing.join(
                arimax_output.select(*ACT_GRP_COLS, "Training_End_Date"),
                [*ACT_GRP_COLS, "Training_End_Date"],
                "left_anti"
            )
            .unionByName(arimax_output)
        )

        updated.write.mode("overwrite").parquet(Arimax_dir)
...
if ets_run:
    ...
    if rerun_historical_forecasts:
        ...
        ## making sure that if there is an existing forecast for the most recent run that it is overwritten
        existing = spark.read.parquet(Holts_dir).cache()
        existing.count()

        updated = (
            existing.join(
                ets_forecasts.select(*ACT_GRP_COLS, "Training_End_Date"),
                [*ACT_GRP_COLS, "Training_End_Date"],
                "left_anti"
            )
            .unionByName(ets_forecasts)
        )

        updated.write.mode("overwrite").parquet(Holts_dir)
