...
## Model Parameters
STEP_SIZE = ML_STEP_SIZE        #pulled from the config file

## The following parameters are set within the config file directly
# FORECAST_HORIZON | SEASONAL_PERIODS | MIN_TRAIN | N_OPTUNA_TRIALS | OPTUNA_SEED | driver_table
...
if run_xgboost:
    if rerun_historical_forecasts:
        ## Writing SHAP OUTPUTS as the model values are consistent
...
if run_xgboost:
    if rerun_historical_forecasts:
        ...
        print(f"appending / replacing new forecast values to the existing parquet {XGB_dir}")
        ## making sure that if there is an existing forecast for the most recent run that it is overwritten
        existing = spark.read.parquet(XGB_dir).cache()
        existing.count()

        updated = (
            existing.join(
                output.select(*ACT_GRP_COLS, "Training_End_Date"),
        ...
        updated.write.mode("overwrite").parquet(XGB_dir)
